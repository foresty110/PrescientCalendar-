...
3. **자동 기록 (ADR / 트러블슈팅)**: 작업 중 다음 신호가 하나라도 보이면 템플릿으로 초안을 작성해 사용자에게 보여주고 승인 후에만 `docs/decisions/` 또는 `docs/troubleshooting/`에 저장한다.
   ...
   - **트러블슈팅 기록**:
     - **`fix` 타입 커밋을 만들 때는 신호·디버깅 시간과 무관하게 항상 트러블슈팅 문서를 기록한다** — trivial 한 수정이어도 예외 없음.
     - `fix` 가 아닌 작업(`refactor`·`feat` 중 고친 버그, 환경 문제 등)은 아래 신호가 하나라도 보이면 기록 (**신호 우선, 시간은 보조**):
       - 표면 증상과 실제 원인이 다름 (예: 로컬 통과 ≠ CI 실패)
       - 한 이슈에 두 개 이상의 수정 시도가 필요했음 (가설→기각→다음 가설)
       - 환경 특유 문제 (Docker, CI, Prisma migrate, OAuth 콜백, deploy)
       - 라이브러리 버그·제약 회피
       - 또는 30분 이상 디버깅
   - **자기 점검**: 각 수정 직후 "이건 표면 증상이었나? 가설을 몇 개 거쳤나?"를 자문. 시간이 짧았더라도 신호가 있으면 로그감.
   - 형식:
   ```
