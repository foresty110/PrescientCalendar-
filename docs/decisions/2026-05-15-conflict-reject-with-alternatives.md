...
초안에선 `force: true` 인자 + UI '그래도 만들기' 우회 경로를 두었으나, 사용자 추가 피드백
("충돌이 절대 있어선 안 돼")으로 **우회 경로를 전부 제거**.

- `CreateEventInput.force` 필드 삭제
- `create_event` Zod 스키마에서 `force` 옵션 제거
- ConflictAlternativesCard 의 '그래도 만들기' 버튼 + 콜백 제거
- 충돌은 항상 거부, 사용자는 대안 카드에서만 선택 가능
- LLM 도 force 호출 경로가 없어 충돌 무시 생성 불가
- 대안 후보 풀 확대 (±15·±30·±45·±60·±90·±120·+180·+24h·+48h) — 빽빽한 일정에서도 3-4개 보장
- ConflictAlternativesCard 가 LLM 텍스트와 중복되지 않도록 Chat 클라이언트에서 충돌 카드 있을 때 텍스트 본문 자동 숨김

이로써 도메인 불변식이 "사용자의 캘린더엔 절대 시간 충돌이 없다" 로 단단해진다.
